...
// === LNN instrumentation (added 2026-09-16) ===
#include "InstrLog.h"
...
Sophus::SE3f Tracking::GrabImageStereo(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const double &timestamp, string filename)
{
    ...
    QualityContext& quality=qualityContext();
...
// === LNN instrumentation (added 2026-09-16) ===
// 采集当前帧的跟踪信号并写入 frame_signals.csv。
// 输出列: frame_id,ts,state,n_kps,n_map_pts,n_inliers,n_lost_kps,
//         reproj_mean_px,reproj_std_px,reproj_median_px,n_reproj,
//         obs_per_mp,n_local_mps,n_local_kfs,is_kf,n_global_kfs,n_global_mps,image_name
// 未设置环境变量 ORB_SLAM3_INSTR_DIR 时立即返回(零开销)。
...
void Tracking::CreateNewKeyFrame()
{
    ...
    // === LNN instrumentation (added 2026-09-16) ===
    // 标记本帧触发了关键帧插入(用于帧级信号里的 is_kf 列)
    mnInstrLastKFId = (int)mCurrentFrame.mnId;
