...
root=Path(__file__).resolve().parent/'quality_gate'
...
(ROOT/'gate_protocol.json').write_text(json.dumps({'candidate_fit_rows':len(fit['y']),'gate_train_rows':len(gc),
 ...
 'signals':'40 observations, motion/IMU consistency and 13 matched frame-quality signals; uninterrupted tracking windows only',
...
def save_gate():
 (ROOT/'results.json').write_text(json.dumps({'records':records,'elapsed_s':time.time()-begun_all,
  'seeds':[0,1,2],'device':DEV,'caveats':['Fresh real-time camera poses, matched quality signals, initial20% GT scale calibration; compensation offline.',
...
class GateNet(nn.Module):
 def __init__(self,kind):
  ...
  if kind.startswith('CfC'):
   self.cell=CfC(gx.shape[2],48,batch_first=True,backbone_units=48,backbone_layers=1)
  ...
  elif kind=='GRU': self.cell=nn.GRU(gx.shape[2],48,batch_first=True)
  else: self.cell=None
  self.head=nn.Sequential(nn.Linear((48 if self.cell is not None else gx.shape[2])+gs.shape[1],48),nn.ReLU(),nn.Linear(48,1))
...
for kind in ['MLP','GRU','CfC_fixed','CfC_time']:
 for seed in [0,1,2]:
  ...
  record_gate(kind+'_conservative',seed,lambda ds:weights_for(ds)*(weights_for(ds)>=cut),{'gate_cutoff':cut})
  torch.save({'state':state,'xm':xm,'xs':xs,'sm':sm,'ss':ss,'unit':unit,'ridge_coef':coef,'ridge_reg':best[1],'physical_gate':alpha,'gate_cutoff':cut},ROOT/f'{kind}_{seed}.pt')
