...
ap.add_argument('--models',default='MLP,GRU,CfC_fixed,CfC_time,CfC_fixed_head,CfC_time_head')
args=ap.parse_args()
...
if args.resume:
 previous=json.loads((OUT/'results.json').read_text()); records=previous['records']; audit=previous['audit']
 start-=previous['elapsed_s']
...
class Net(nn.Module):
 def __init__(self,kind,dim):
  ...
  if kind.startswith('CfC'):
   self.cell=CfC(dim,AutoNCP(64,16 if kind.endswith('_head') else 3),batch_first=True)
   if kind.endswith('_head'): self.head=nn.Linear(16,3)
 ...
 def forward(self,x,ts):
  if self.kind.startswith('CfC'):
   z,_=self.cell(x,timespans=ts if self.kind.startswith('CfC_time') else None)
   return self.head(z[:,-1]) if self.kind.endswith('_head') else z[:,-1]
...
def run_case(label,train_ds,tr,va,test_sets,configs):
 ...
 for cfg in configs:
  ...
  def record(kind,seed,test_name,ds,te,pred,detail=None):
   if any(r['case']==label and r['config']==cfg and r['model']==kind and r['seed']==seed and r['test_sequence']==test_name for r in records): return
  ...
  dual=len(z)<z.shape[1]
  gram=z@z.T if dual else z.T@z
  e,u=np.linalg.eigh(gram.astype('float64'))
  uy=u.T@(yn[tr] if dual else z.T@yn[tr])
  best=None
  for alpha in [0.1,1,10,100,1000]:
   coef=u@(uy/(e[:,None]+alpha))
   if dual: coef=z.T@coef
  ...
  for kind in args.models.split(','):
   for seed in SEEDS:
    if all(any(r['case']==label and r['config']==cfg and r['model']==kind and r['seed']==seed and r['test_sequence']==test_name for r in records) for test_name,_,_ in test_sets): continue
