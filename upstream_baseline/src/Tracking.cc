...
void Tracking::LogFrameSignals()
{
    ...
    instr::FrameSignal sig;
    sig.frameId  = (int)mCurrentFrame.mnId;
    sig.ts       = mCurrentFrame.mTimeStamp;
    sig.state    = (int)mState;
    sig.nKps     = mCurrentFrame.N;
    sig.nInliers = mnMatchesInliers;
    sig.imgName  = mCurrentFrame.mNameFile;

    // --- 当前帧关联地图点: 数量 / 观测次数 / 重投影误差(像素) ---
    std::vector<float> errs;
    errs.reserve(mCurrentFrame.N);
    double obsSum = 0.0;
    int    obsCnt = 0;
    ...
    for(int i = 0; i < mCurrentFrame.N; ++i)
    {
        ...
        sig.nMapPts++;
        obsSum += (double)pMP->Observations();
        obsCnt++;

        if(mCurrentFrame.mvbOutlier[i])
            continue;

        Eigen::Vector3f Pc = mCurrentFrame.GetPose() * pMP->GetWorldPos();
        if(Pc(2) <= 0.0f)
            continue;

        Eigen::Vector2f uv = mCurrentFrame.mpCamera->project(Pc);
        const cv::Point2f kp = mCurrentFrame.mvKeysUn[i].pt;
        const float dx = uv(0) - kp.x;
        const float dy = uv(1) - kp.y;
        errs.push_back(std::sqrt(dx*dx + dy*dy));
    }

    instr::DistStats st = instr::distStats(errs);
    sig.nReproj      = st.n;
    sig.reprojMean   = st.mean;
    sig.reprojStd    = st.stdv;
    sig.reprojMedian = st.median;
    sig.obsPerMp     = (obsCnt > 0) ? (obsSum / obsCnt) : 0.0;
    sig.nLostKps     = mCurrentFrame.N - sig.nMapPts;

    // --- 局部地图规模(仅Tracking线程写入，无竞争) ---
    sig.nLocalMps = (int)mvpLocalMapPoints.size();
    sig.nLocalKfs = (int)mvpLocalKeyFrames.size();

    // --- 全局地图规模 ---
    if(mpAtlas)
    {
        sig.nGlobalKfs = (int)mpAtlas->GetAllKeyFrames().size();
        sig.nGlobalMps = (int)mpAtlas->GetAllMapPoints().size();
    }

    // --- 本帧是否插入关键帧 ---
    sig.isKf = (mnInstrLastKFId == (int)mCurrentFrame.mnId) ? 1 : 0;
