...
void Optimizer::LocalBundleAdjustment(KeyFrame *pKF, bool* pbStopFlag, Map* pMap, int& num_fixedKF, int& num_OptKF, int& num_MPs, int& num_edges)
{
    ...
    optimizer.initializeOptimization();
    // === LNN instrumentation (added 2026-09-16) ===
    std::chrono::steady_clock::time_point instr_tOptStart = std::chrono::steady_clock::now();
    optimizer.optimize(10);
    const double instr_optMs = std::chrono::duration_cast<std::chrono::duration<double,std::milli> >(
                                   std::chrono::steady_clock::now() - instr_tOptStart).count();

    // === LNN instrumentation (added 2026-09-16) ===
    // 统计本次 Local BA 的重投影误差分布(像素)与外点比例。
    // 说明: chi2 未超阈值的边不受 Huber kernel 缩放, 其 error().norm() 即真实像素重投影误差。
    if(instr::files().active)
    {
        std::vector<float> pixErrs;
        ...
        for(size_t i = 0; i < vpEdgesMono.size(); ++i)
        {
            ...
            if(!e) continue;
            const double c2 = e->chi2();
            if(std::isfinite(c2)) chi2s.push_back((float)c2);
            if(c2 <= 5.991 && e->isDepthPositive())
            {
                ++nInlierEdges;
                const double en = e->error().norm();
                if(std::isfinite(en)) pixErrs.push_back((float)en);
            }
        }
        // 相机-机体外参观测
        ...
        b.kfId       = (int)pKF->mnId;
        b.ts         = pKF->mTimeStamp;
        b.nFixedKf   = num_fixedKF;
        b.nOptKf     = num_OptKF;
        b.nMps       = num_MPs;
        b.nEdges     = num_edges;
        b.nInlierEdges = nInlierEdges;
        instr::DistStats cs = instr::distStats(chi2s);
        b.chi2Mean   = cs.mean;
        b.chi2Std    = cs.stdv;
        instr::DistStats ps = instr::distStats(pixErrs);
        b.pixMean    = ps.mean;
        b.pixStd     = ps.stdv;
        b.pixMedian  = ps.median;
        b.pixP90     = ps.p90;
        b.optMs      = instr_optMs;
        instr::logBA(b);
    }
...
void Optimizer::LocalInertialBA(KeyFrame *pKF, bool *pbStopFlag, Map *pMap, int& num_fixedKF, int& num_OptKF, int& num_MPs, int& num_edges, bool bLarge, bool bRecInit)
{
    ...
    // === LNN instrumentation (added 2026-09-16, VI) ===
    // 统计本次 LocalInertialBA 的"跨模态不一致": g2o::EdgeInertial 残差
    //   _error = [er(3, rad), ev(3, m/s), ep(3, m)]   (IMU预积分 vs 视觉估计)
    // 这个量在线可算(不需要 GT), 是"视觉-惯性不一致能否预测漂移"的核心特征。
    if(instr::files().active)
    {
        double sR=0,sR2=0,mR=0, sV=0,sV2=0,mV=0, sP=0,sP2=0,mP=0, sC=0,mC=0;
        int nR=0,nV=0,nP=0,nC=0,nGRW=0,nARW=0;
        for(int i=0;i<N;i++)
        {
            if(!vei[i]) continue;
            vei[i]->computeError();
            const auto& e = vei[i]->error();
            double nr = e.template segment<3>(0).norm();
            double nv = e.template segment<3>(3).norm();
            double np = e.template segment<3>(6).norm();
            sR += nr; sR2 += nr*nr; if(nr>mR) mR=nr; nR++;
            sV += nv; sV2 += nv*nv; if(nv>mV) mV=nv; nV++;
            sP += np; sP2 += np*np; if(np>mP) mP=np; nP++;
            double c2 = vei[i]->chi2();
            sC += c2; if(c2>mC) mC=c2; nC++;
            if(vegr[i]) nGRW++;
            if(vear[i]) nARW++;
        }
        instr::VISignal v;
        ...
        v.ts            = pKF->mTimeStamp;
        v.nInertialEdges= nR;
        v.nVisEdges     = (int)(vpEdgesMono.size()+vpEdgesStereo.size());
        if(nR>0){ double mu=sR/nR, va=sR2/nR-mu*mu; if(va<0) va=0;
                  v.errRMean=mu; v.errRStd=std::sqrt(va); v.errRMax=mR; }
        if(nV>0){ double mu=sV/nV, va=sV2/nV-mu*mu; if(va<0) va=0;
                  v.errVMean=mu; v.errVStd=std::sqrt(va); v.errVMax=mV; }
        if(nP>0){ double mu=sP/nP, va=sP2/nP-mu*mu; if(va<0) va=0;
                  v.errPMean=mu; v.errPStd=std::sqrt(va); v.errPMax=mP; }
        if(nC>0){ v.chi2InertialMean=sC/nC; v.chi2InertialMax=mC; }
        v.relRobustChi2 = (err>0.0f) ? (double)(err_end/err) : 0.0;
        v.nGyroRW = nGRW; v.nAccRW = nARW;
        Eigen::Vector3f bg = pKF->GetGyroBias();
        Eigen::Vector3f ba = pKF->GetAccBias();
        Eigen::Vector3f vv = pKF->GetVelocity();
        v.bgx=bg(0); v.bgy=bg(1); v.bgz=bg(2);
        v.bax=ba(0); v.bay=ba(1); v.baz=ba(2);
        v.vx =vv(0); v.vy =vv(1); v.vz =vv(2);
        instr::logVI(v);
    }
